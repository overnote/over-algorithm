...
// 增：按照一个位置插入数据
bool InsertElem(SqList &L, int idx, ElemType e) {
...
bool DeleteElem(SqList &L, int idx, ElemType &e) {
...
// 改
bool UpdateElem(SqList &L, int idx, ElemType e) {
    if (idx < 1 || idx > L.length) {
        printf("修改位置不合法\n");
        return false;
    }
...
// 按值查找
bool LocateElem(SqList L, ElemType e, int &idx) {
    for (int i = 0; i < L.length; i++) {
        if (L.data[i] == e) {
            idx = i + 1;
            return true;
